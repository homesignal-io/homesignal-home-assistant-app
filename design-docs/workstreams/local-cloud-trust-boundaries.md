...
Local/cloud trust boundaries define what the cloud may request, what the local add-on or future supervisor may execute, and which authority checks must happen before host-affecting behavior.
...
- Treat command ACK as add-on accepted/rejected, not transport receipt.
...
V0 does not provide arbitrary remote control of devices behind the Home Assistant gateway. The cloud may request only allowlisted device actions that are explicitly modeled by service specs, authorized by the control plane, delivered through the approved device transport for that action, and accepted by the local add-on policy gate.

The cloud can set product intent, but it cannot directly reach through Home Assistant as an unrestricted operator. Local execution remains bounded by add-on capabilities, Home Assistant/Supervisor permissions, local policy, command type, and safety checks.

Allowed or expected v0 host-affecting cases:

- HomeSignal add-on update intent/status flow through the release/update architecture
...
- health/status collection from Home Assistant, Supervisor, add-on runtime, storage, backup, update, and connectivity signals
...
- User/integrator-approved rollback is a normal platform capability for HomeSignal add-on releases.
- Automatic rollback is allowed for failed HomeSignal-controlled add-on update attempts when bounded health/startup/reconnect checks fail.
...
- Bounded HomeSignal add-on diagnostics and temporary debug capture do not
  require local user approval.
- They do require cloud-side authorization, audit, TTL, redaction, size/rate
  limits, and add-on allowlists.
- This covers HomeSignal add-on runtime status, HomeSignal cloud/API
  connectivity, AWS IoT connectivity, add-on version, policy version, update
  status, backup summary, redacted HomeSignal add-on log excerpts, and bounded
...
- V0 diagnostics may include explicit, bounded commands such as `collect_addon_status`, `collect_connectivity_check`, `collect_recent_error_excerpt`, `collect_update_readiness`, `collect_backup_status`, and `request_error_log_bundle`.
- V0 diagnostics must be scoped to HomeSignal add-on/runtime health, cloud connectivity, local update readiness, backup status, and redacted log excerpts.
- Broad Home Assistant configuration snapshots are not v0 diagnostics.

Future support-action optionality:

- The add-on may ship with a dormant, deny-by-default support-action framework
  so future support capabilities do not always require an add-on update.
- Dormant support actions must be declared as local capabilities with stable
  names, minimum add-on version, required local permission surface, redaction
...
- Future broad Home Assistant or host-affecting support actions still require a
  product/security decision before they are enabled, even if the add-on already
...
- V0 command policy and command-shape defaults are now locked:
  ...
  - Local command policy is allowlisted per command type and destination, persisted in add-on/Cloud policy record as a compact allowlist, and rejects everything else by default.
...
- Future local supervisor responsibility split is still a future product consideration; v0 assumes add-on-host boundary for bounded local execution.
