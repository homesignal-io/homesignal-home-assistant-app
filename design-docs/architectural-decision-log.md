...
## 2026-05-18 - Home Assistant App Repository And Release Tracks

Status: Resolved

Outcome:
HomeSignal has one product source repository. Home Assistant app repositories
are public, thin distribution outputs generated by CI, not product authority or
secret configuration. The default distribution strategy uses separate public
install channels for production stable, production candidate/test cohort, and
staging/non-prod. Stable and candidate are both production because both pair to
production HomeSignal and may handle real customer data; staging points only at
staging cloud.

HomeSignal cannot silently force an arbitrary percentage of existing Home
Assistant installs onto a different repository. Cohort steering happens through
the pairing/invite install link for new installs, explicit local channel changes
for existing installs, and cloud release intent/observation after the app
reports its installed slug, track, version, and update posture. Home Assistant
Supervisor remains the local install/update executor.

Canonical locations:
- `design-docs/ha-app-repository-release-strategy.md`
...
## 2026-05-18 - Staging Domain And First Deploy Phasing

Status: Resolved

Outcome:
The first staging deploy has two phases. Stage 0 may use a generated AWS
endpoint and no custom DNS to prove the low-cost control-plane skeleton,
scripts, IaC, logs, and smoke endpoints. Stage 0 is not a real browser/product
staging origin.

Before enabling public pairing pages, browser localStorage/postMessage bridge
behavior, Home Assistant app staging environment profiles, claim flows, email
links, or customer/integrator-visible staging UX, HomeSignal must use a stable
owned HTTPS domain. If no domain exists yet, continue with Stage 0 only, then
wire the staging domain before treating pairing, claim, or browser-origin work
as real staging.

The Home Assistant app must not compensate for missing DNS with arbitrary cloud
URL fields, hidden URL-parameter unlocks, or generated AWS endpoints in durable
environment config. Runtime cloud targets come from allowlisted environment
profiles.
...
## 2026-05-18 - Home Assistant App Terminology And Schema Names

Status: Resolved

Outcome:
HomeSignal uses Home Assistant app terminology for the local Home Assistant
runtime surface. Product prose, architecture docs, mock UI labels, repository
metadata, Go module paths, and owned schema fields must use the Home Assistant
app name. Schema and contract names that refer specifically to this boundary
must use the `ha_app` prefix, for example `ha_app_version`,
`latest_ha_app_version`, `ha_apps`, and `ha_app_id`, so they are not confused
with the web app, control-plane runtime, or generic application concepts.

Compatibility literals owned by Home Assistant remain unchanged when changing
them would break installation or runtime behavior, including `addon_config:rw`,
the current My Home Assistant repository redirect identifier, and Supervisor
ingress paths under `/hassio/addon/...`.

Canonical locations:
- `README.md`
- `repository.yaml`
- `homesignal/config.yaml`
- `design-docs/app-skeleton.md`
- `design-docs/ha-app-runtime-architecture.md`
- `design-docs/ha-app-ui-mount-plan.md`
...
- `design-docs/home-assistant-app-backend-reconciliation.md`
- `design-docs/app-runtime-error-and-artifact-contract.md`
...
Outcome:
Device enrollment moved from app-created six-digit pairing codes to
...
emails the raw code once and stores only a hash/fingerprint. The local app
verifies the code first and receives integrator, creator, site, and customer
display details plus a short verification token. The app can commit the
...
rate-limited and audited. The public app enrollment endpoints are POST-only,
...
- `design-docs/ha-app-user-journey-report.md`
- `design-docs/home-assistant-app-backend-reconciliation.md`
- `design-docs/app-runtime-error-and-artifact-contract.md`
...
Assistant Core/Supervisor/backup/storage facts, `ha_apps` as an array of
observed Home Assistant app objects, and `runtime_log_summary` for collapsed,
...
Outcome:
HomeSignal is not irrevocable MDM for a Home Assistant installation. A local Home Assistant administrator may remove the app or reset HomeSignal local identity and create a fresh claim with another account/integrator. The web claim flow owns repair/reconnect/fresh-claim choices based on authenticated user, account, site, prior-match context, and authorization; cross-account matches must not expose or mutate prior account records without authority.
...
Outcome:
CI uses simulator contract tests for changed runtime contracts. Staging must include at least one long-lived real Home Assistant/app canary paired with staging AWS IoT Core. Ephemeral Home Assistant/app instances are used for enrollment, claim, repair/reconnect, and fresh-claim lifecycle tests.
...
Outcome:
The cloud may request only modeled, allowlisted local actions over AWS IoT. Local execution policy means app-side safety policy, not default per-command human approval.
...
Outcome:
HomeSignal app rollback is a normal platform capability, not a hidden support-only action. User/integrator-approved rollback is allowed for supported HomeSignal app releases. Automatic rollback is allowed only for failed HomeSignal-controlled app update attempts that fail bounded local health/startup/reconnect checks, and it must not mutate broader Home Assistant, Supervisor, OS, database, or arbitrary host state. V0 app update execution remains through the local Home Assistant app/Supervisor release path; HomeSignal may publish desired version/channel intent and observe/report status.

V0 diagnostics are scoped to bounded HomeSignal app/runtime checks and redacted log/error collection. Broad Home Assistant configuration snapshots are not v0 diagnostics.
...
Temporary debug mode is the targeted escape hatch. It is cloud-initiated by an authorized internal/support actor, not customer self-service. Diagnostics owns debug-session product behavior; API Facade owns route shape; Authorization Service owns permission checks; Command Service owns device command lifecycle; Artifact Upload Broker owns debug bundle transfer; the app enforces TTL, redaction, local rate/size limits, and safe capture categories.
...
or temporary support override without changing the app contract.
...
The v0 app compatibility window is current protocol family plus one prior
...
app versions should be visible in the UI.
...
Outcome:
Bounded v0 HomeSignal app diagnostics and temporary debug capture do not
require local user approval. They require cloud authorization, audit, TTL,
redaction, size/rate limits, and app allowlists. Future broad Home Assistant
...
product interpretation. The app performs or reports local backup facts. V0
...
reworking the app/cloud contract, but v0 does not build pricing or UX around
live event streams.
...
The app may include dormant, deny-by-default support-action handlers for
...
show simple device/app health derived from latest telemetry and presence.
...
App local state migrations are versioned, idempotent, atomically written,
...
Update wording was aligned with the update architecture: v0 HomeSignal app
updates are desired version/channel intent plus status observation through the
Home Assistant Supervisor/app release path. HomeSignal does not initiate
...
The desired HomeSignal app version is explicitly modeled as
`homesignal_edge.update` shadow desired state. CI/CD publishes the app
release through the normal GitHub/Home Assistant app release channel first;
...
`backup_failed`, `backup_overdue`, `app_update_attention`,
`ha_update_advisory`, and `storage_warning`. `app_update_attention` uses a
...
## 2026-05-17 - Home Assistant App Runtime, UI, And Local Logging Boundary

Status: Resolved

Outcome:
The Home Assistant app now has one implementation guide for the mounted UI,
local Go runtime boundaries, local diagnostic logging, log-level controls,
bounded local log retention, and cloud-authorized app log retrieval. The
...
selectors, but never by arbitrary local file path.
