...
The current repository implements the Home Assistant app and enrollment client contract only. Most cloud services in this map are target architecture, not code that exists in this repo today.
...
flowchart TB
  ...
  App -- "HTTPS enrollment only before/while claiming" --> API
  Enrollment -- "CreateCertificateFromCsr + bind Thing/policy" --> ProvisioningAPI
  ProvisioningAPI --> Registry
  App -- "claimed-device telemetry/events over mTLS" --> TelemetryIngest
  App <-->|desired/reported edge state| Shadows
...
| Phase | Services |
| --- | --- |
| Current in this repo | HomeSignal Manager app, local enrollment state, local `/ui`, `/status`, `/readyz`, mockable HomeSignal enrollment client, mockable AWS provisioning client |
...
| Next AWS step | AWS IoT provisioning adapter for `CreateCertificateFromCsr`, Thing/certificate binding, IoT policy attachment, and real app CSR submission/storage |
| MVP after enrollment | Separately deployable Telemetry Ingest, IoT lifecycle presence, basic telemetry/events, Edge State Adapter module, portal device status, release/revocation, backup status/trigger, HomeSignal app update intent/status |
| MVP portal completion | Customer-facing email alerts for disconnected devices, backup failures, and app/update attention using Alerting Service plus Notification Service with a Resend-backed transactional email provider adapter |
...
| Backup Service | V0 logical service for backup trigger requests, backup policy/status records, command/result interpretation, last success/failure, overdue/in-progress state, backup summary metadata, and offsite Home Assistant backup artifact product meaning | Running arbitrary local backup code outside the app or generic artifact transfer |
...
| Store | Data | Notes |
| --- | --- | --- |
| App `/config/device.json` | Installation ID, claim state, device ID, IoT Thing name, cert/key paths, pending metadata | Metadata only; no secret contents |
| App `/config/secrets/*` | Poll token and temporary enrollment/session material | `0600`; temporary material removed after use/expiry |
| App `/config/iot/*` | Local private key and final device certificate | `0600`; private key never leaves device |
| App local queue | Future bounded offline reports/ACKs | Prevents cloud loss from breaking local HA operation |
...
sequenceDiagram
  ...
  App->>API: Verify claim invite code (installation_id, recognition, CSR hash)
  API->>DB: Validate invite, creator authority, expiry, uniqueness; audit
  API-->>App: integrator/site/customer details + verification token
  App->>API: Confirm claim verification (token, details hash, CSR)
  ...
  API-->>App: claimed confirmation + certificate PEM + IoT config
```

The app enters `CLAIMED` only after HomeSignal finalization confirms the claim.

Direct app-to-HomeSignal API calls are limited to enrollment bootstrap, claim finalization, and approved `/agent/*` mTLS flows. Once a device is claimed, realtime control/wake-up traffic should use AWS IoT Core as the device transport.
...
sequenceDiagram
  ...
  HA-->>App: Local status, versions, backup/update summaries
  App->>AgentAPI: POST /agent/telemetry or /agent/events with mTLS
...
Telemetry originates at the app and uses the mTLS Agent HTTPS API in v0. AWS
...
and app/update attention. Each email recipient may have independent alert
...
sequenceDiagram
  ...
  Shadows-->>App: Delta/current desired state
  App->>Shadows: Report applied/observed edge state
...
V0 uses one named shadow, `homesignal_edge`, with only `publish_policy` and `update` sections. The shadow carries version/reference pointers plus tiny resolved config values the app needs to converge immediately. Convergence projection is internal/support-visible in v0.
...
sequenceDiagram
  ...
  Broker-->>App: Deliver notification if connected
  ...
  App->>AgentAPI: ACK accepted/rejected, optional progress, terminal result over mTLS
...
Command lifecycle is documented in `command-lifecycle.md`. ACK means the app accepted or rejected the command within the ACK window, not mere transport receipt. Progress updates are opt-in by command type; terminal results are separate for long-running work.

Publish-policy desired state should flow through the Edge State Adapter and AWS IoT named shadows. A notification can still be a cheap "policy changed" hint, and `refresh_publish_policy` can still be an ACK-required repair/acceleration command when cloud wants a bounded attempt. The app must allowlist command types and reject unknown or unsafe operations. If policy application fails, the command result or shadow-reported convergence status records the bounded reason and the app emits a bounded internal `agent_alarm`; the event may include a redacted diagnostic excerpt capped at 5 KB total and a `more_logs_available` hint. Larger logs are handled through cloud-authorized artifact upload, not MQTT. Upload failures do not recursively request more log uploads. Missing ACK/result behavior, retry policy, timeout recording, and internal alert candidates are owned by `command-lifecycle.md`.
...
flowchart LR
  ...
  Broker --> App["App"]
  App --> AgentAPI["Agent HTTPS API (mTLS)"]
...
backup appears overdue or in progress. The Home Assistant app performs or
...
flowchart LR
  ...
  Release --> Repo[("GitHub / HA app release channel")]
  DB --> EdgeState["Edge State Adapter / homesignal_edge.update"]
  EdgeState --> App["App / future local supervisor"]
  Repo --> App
  App --> Health["Local health checks"]
  Health --> App
```

CI/CD publishes the HomeSignal app version through the normal GitHub/Home Assistant app release channel. The cloud then declares desired version and rollout intent through `homesignal_edge.update` when a cohort should converge. In v0, HomeSignal does not initiate binary installation over IoT Core; the Home Assistant Supervisor/app release path performs installation according to local policy while HomeSignal observes reported version, health, and terminal status. Command-class update work is limited to explicitly modeled bounded checks, status, or repair. A future local supervisor can harden and expand local convergence, backup/snapshot checks, health checks, and rollback.
...
### App to HomeSignal API, Enrollment Only
...
- **HomeSignal API unavailable before claim:** app stays local-safe and `UNCLAIMED`; no fake code.
- **Claim invite expires:** app cannot verify/confirm the invite and asks for a new code from the integrator; GC marks the invite expired and expires active verifications.
- **Claim verification expires:** app abandons pending confirmation unless AWS provisioning already succeeded.
- **AWS IoT provisioning fails:** app remains `PAIRING_PENDING` or degraded and does not claim.
- **AWS succeeds but HomeSignal finalization fails:** app remains degraded `PAIRING_PENDING`; no runtime messaging.
- **Cloud unavailable after claim:** local Home Assistant remains unaffected; cloud-facing telemetry/commands degrade.
- **AWS IoT publish path unavailable:** app buffers bounded non-dangerous reports and reconnects with backoff/jitter.
...
- **Update failure:** app or future local supervisor rolls back or remains degraded without breaking Home Assistant operation.
...
3. Add claim confirmation/finalization with fake AWS IoT provisioning first. Defer post-claim device status/revocation checks unless a later app/API spec reintroduces them.
...
7. Add v0 backup status/trigger and HomeSignal app update intent/status flows through the command/update architecture.
